...
const preguntas = [
  ...
  {
    text: "Me resulta comodo hablar frente a varias personas.",
...
function showPregunta(index) {
  ...
  btn.className = "btn";
  btn.textContent =
    index === preguntasFiltradas.length - 1 ? "Ver Resultados" : "Siguiente";
...
function actualizarProgreso() {
  const percent = Math.round(
    (preguntaActual / preguntasFiltradas.length) * 100
  );
...
function showResults(puntajes) {
  resultsDiv.innerHTML =
    '<h2 style="color: #fff;">Tus mejores oportunidades academicas</h2>';
  testForm.classList.add("hidden");
  progressBar.classList.add("hidden");

  const preguntasPorCarrera = {};
  carreraSeleccionada.forEach((carrera) => {
    preguntasPorCarrera[carrera] = preguntas.filter((q) =>
      q.carreras.includes(carrera)
    ).length;
  });
...
function showResults(puntajes) {
  resultsDiv.innerHTML =
    '<h2 style="color: #fff;">Las carreras que más coinciden con tus intereses son:</h2>';
  testForm.classList.add("hidden");
  progressBar.classList.add("hidden");
